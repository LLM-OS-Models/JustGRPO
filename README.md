...
**예상 일정 (한국시간)**: Run 5 **7/27 새벽–오후** 종료·평가 → Run 6 **7/28 중** →
Run 7/8 순차 → **전체 매트릭스 완성 7/29 목표**. (MDLM 논문 방식은 원저자도 16×H100을 쓴
무거운 레시피 — 우리는 1×H100이라 무손실 가속을 총동원한 수치. 실측 페이스로 계속 갱신.)
...
| 가속 | 방법 | 검증 |
|---|---|---|
| fast rollout (~1.5–2×) | 버려지는 위치의 152k-vocab 투영 생략 (`utils/mdlm_fast.py`) | greedy 출력 토큰 단위 일치 |
| rollout 16개 단일 배치 (~1.4×) | 2×8 호출을 1×16으로 — 같은 그룹 수학 | 의미상 동일 (같은 프롬프트 그룹 16) |
| 손실 위치 배칭 K=4 (~2×) | 원본 256회 루프를 4위치씩 배치 | fp32 오차 1.2e-5 |
| 단독 실행 | OOM 경합 제거 | — |
