...
|---|---:|---:|---:|---:|
| 베이스 | 29.8 | 30.5 | 29.2 | 8.8 |
| Run 5 (수학 단독) | 34.72 **(+4.9)** | 30.49 (~0) | 30.20 (+1.0) | **12.28 (+3.5)** |
| **Run 6 (믹싱)** | **36.77 (+7.0)** | 19.51 (−11.0)* | 12.80 (−16.4)* | 측정중 |

**두 가지 상반된 발견:**

1. **✅ 믹싱이 수학(GSM8K)에서 단독을 이긴다** — GSM8K를 1/3 빈도로만 학습했는데도 +7.0 > 단독 +4.9.
   코드·수학응용의 긍정적 교차전이. 논문 방식이 0.6B에서 작동 + 멀티도메인 > 단일도메인(수학 한정).

2. **⚠️ (*) 믹싱의 코드 급락은 "능력 저하"가 아니라 "장황함에 의한 잘림"** — 생성물 직접 검증 결과,
   믹싱 모델은 긴 docstring·주석부터 쓰다가 256토큰 예산 안에 함수 본문을 못 끝내 잘린다
   (400토큰이면 구현을 쓰기 시작 → 능력은 살아있음). 원인: 믹싱에 든 **MATH 경시수학 긴 CoT가
   모델을 장황화**시켜 코드에도 전이. 단독(GSM8K 짧은 답)은 장황화가 덜해 코드 보합.
   → 완화책: 더 긴 gen_length 또는 간결성 보상 (후속 검증). **"믹싱이 코드를 파괴한다"는 부정확한 서술.**

(MATH 비교는 측정 중. 단독은 bd3lm과 달리 MDLM에서 코드를 안 깎음 — 앞서 본 파괴적 간섭의 상당 부분이
bd3lm mismatch 탓이었음이 재확인됨. 아래는 Phase 1 bd3lm의 목표별 판정 — mismatch가 섞인 조건.)
