"""Paper-identical JustGRPO training for MDLM-style checkpoints (Run 5+).

Copy of train.py wired to grpo_mdlm.py (original LLaDA recipe: AR-order rollout
via utils/generate.py + per-token bidirectional loss). Kept as a separate file so
the BD3LM pipeline (train.py / grpo.py) stays untouched and both remain
...
@dataclass
class TrainConfig:
    """Training hyperparameters for GRPO (MDLM / paper-identical path)."""

    # --- Model ---
    model_path: str = "/home/ubuntu/data/models/Qwen3-0.6B-diffusion-mdlm-v0.1"
    ...
    loss_chunk: int = 4  # t-positions batched per loss forward (math-identical)
...
def train(config: TrainConfig):
    ...
    print("=" * 60)
    print("JustGRPO Training (MDLM, paper-identical recipe)")
    print("=" * 60)
...
def parse_args():
    parser = argparse.ArgumentParser(description="JustGRPO Training (MDLM, paper-identical)")

    parser.add_argument("--model_path", type=str, default=None, help="Model path or HF id (default: local Qwen3-0.6B MDLM)")
    ...
    parser.add_argument("--dataset", type=str, default="gsm8k", help="Dataset: gsm8k, math, code, mixed")
    ...
    parser.add_argument("--save_every", type=int, default=10, help="checkpoint save interval (steps)")
