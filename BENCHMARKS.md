...
|---|---:|---:|---:|---:|
| Run5 수학단독 | 34.72 (+4.9) | 30.49 (~0) | 30.20 (+1.0) | 12.28 (+3.5) |
| **Run6 믹싱** | **36.77 (+7.0)** | 19.51 (−11.0) ⚠️ | 12.80 (−16.4) ⚠️ | 측정중 |

**규명 완료 (생성물 직접 검증)**: 코드 급락은 **능력 저하가 아니라 "장황함에 의한 잘림"**이다.
- 믹싱 모델은 코드 프롬프트에 긴 docstring·주석부터 쓰다가 256토큰 안에 함수 본문을 못 끝내고 잘림 → 테스트 실패.
- 400토큰으로 늘리면 실제 구현(sort/반복 등)을 쓰기 시작 → 코딩 능력은 살아있음을 확인.
- 원인: 믹싱에 포함된 **MATH(경시수학 긴 CoT) 데이터가 모델을 장황하게** 만들었고, 그 장황함이 코드에도 전이되어
  고정 생성예산(256) 안에서 코드가 잘림. 단독(GSM8K, 짧은 답)은 장황화가 덜해 코드가 보합.
- **함의**: 믹싱의 코드 하락은 형식/길이 상호작용이며, 더 긴 gen_length 또는 간결성 보상으로 완화 가능(후속 검증 후보).
  → "믹싱이 코드 능력을 파괴한다"가 아니라 "장황화가 짧은 예산에서 코드를 잘리게 한다"가 정확한 서술.
