...
| GSM8K | 베이스 | 학습 후 | 향상 | GSM8K 노출 |
|---|---:|---:|---:|---|
| Run 5 (수학 단독) | 29.8 | 34.72 | +4.9 | 100% |
| **Run 6 (믹싱)** | 29.8 | **36.77** | **+7.0** | 33% |

**믹싱이 단독의 홈그라운드(GSM8K)에서 단독을 +2.05 이겼다.** GSM8K를 1/3 빈도로만 학습했는데도 더 높다
→ 코드·수학응용 데이터의 긍정적 교차전이. **논문 방식이 0.6B에서 작동함 + 멀티도메인 > 단일도메인**을 동시 실증.
(나머지 3태스크 비교는 GPU 여유 시 채움. 아래는 Phase 1 bd3lm의 목표별 판정 — mismatch가 섞인 조건.)
