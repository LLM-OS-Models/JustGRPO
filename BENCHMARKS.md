...
### ckpt-100 4태스크 비교 (진행) — ⚠️ 믹싱 코드 급락 (검증 중)

MDLM 베이스: GSM8K 29.8 · MATH 8.8 · HumanEval 30.5 · MBPP 29.2

| ckpt-100 | GSM8K | HumanEval | MBPP | MATH |
|---|---:|---:|---:|---:|
| Run5 수학단독 | 34.72 (+4.9) | 30.49 (~0) | 30.20 (+1.0) | 측정중 |
| **Run6 믹싱** | **36.77 (+7.0)** | 19.51 (−11.0) ⚠️ | 12.80 (−16.4) ⚠️ | 측정중 |

**예상 밖 결과**: 믹싱은 코드 데이터를 33% 포함하는데도 코드(HumanEval/MBPP)가 단독보다 크게 낮다.
단독(코드 미학습)은 코드 보합인데, 믹싱(코드 학습)은 코드 급락 — 직관에 반함.
→ 실제 능력 저하인지 출력 형식/추출 아티팩트인지 **생성물 검증 진행 중.** 결론 유보.
